...
The images are scaled and aligned to fit in with surrounding text,
respecting baselines and ascent.  The LaTeX source for an image is
available as its alt-text in the HTML document.

#+vindex: org-html-latex-image-options
The apperance and inclusion method of LaTeX preview images in HTML
exports is governed by the option ~org-html-latex-image-options~.  In
particular, the =:image-dir= key controls the absolute or relative
path where LaTeX preview images are stored.
