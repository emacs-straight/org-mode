...
~org-latex-multi-lang-driver~; for build sequences, see
...
options. The LaTeX export backend has several additional keywords to
...
- =LATEX_MULTI_LANG= ::
  ...
  #+vindex: org-latex-multi-lang-driver
  #+vindex: org-export-default-language
...
in a first stage and that will be then translated to
...
=polyglossia= will load =fontspec= automatically.
...
initialisation code, in the file local variables or in the [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Directory-Variables.html][directory
...
is the default value).  This is normally the case when exporting to
=beamer=, where you most probably want to use the fonts defined in the
template.
...
If you are using =lualatex= as your LaTeX compiler, you can also declare
fallback fonts for specific fonts when specific character scripts are
detected by Emacs during the export process.

Use the property ~:fallback~ for this purpose.  It links to an
associative list that maps the script names as detected by Emacs with
their fallback configuration.  Take, for example, that you configure the
=main= font in ~org-latex-fontspec-config~ as:
...
When you specify ~:features~ for a font, the fallback is appended as
another feature to the font definition.
...
definitions for the different fonts you need in your PDF output.
Each definition is a property list (~plist~).

=polyglossia= controls fonts on a per-language, per-script basis. For
example, using the =FreeMono= script for Hebrew script in mono-spaced
passages is achieved with the following definition in LaTeX:

#+BEGIN_SRC LaTeX :results none :exports code
  \newfontfamily{\hebrewfonttt}[Script=Hebrew]{FreeMono}
#+END_SRC

To include the definition for the monospaced Hebrew font above in
~org-latex-polyglossia-font-config~, you will need to add the following

#+BEGIN_SRC emacs-lisp :results none :exports code
  ("he"  :font "FreeMono" :variant "tt" :props "Script=Hebrew")
#+END_SRC

to it. Each entry associates a language, ="he"= for Hebrew[fn:51] to a list
of properties.
...
:description: Using org-latex-babel-fontspec
:END:
#+cindex: babel font configuration
#+vindex: org-latex-babel-fontspec

Org mode will control the fonts used for multilingual documents when
using the =babel= package with the ~org-latex-babel-fontspec~
...
property list array mapping babel language names to the fonts used
when exporting to babel.  Each entry maps a language name to a
~:fonts~ property.  Use ~"AUTO"~ as the language name for the fonts
...
and ~FONT~ is the system font name.

For example, for a document with text in Latin and Arabic script,
where the main language uses a Latin script, this variable could have
the following value:

#+BEGIN_SRC elisp :results none :exports code
'(("AUTO" :fonts (("rm" . "Noto Serif")
                  ("sf" . "Noto Sans")
                  ("tt" . "Noto Mono")))
  ("arabic" :fonts (("rm" . "Noto Naskh Arabic")
                    ("sf" . "Noto Kufi Arabic"))))
#+END_SRC

provided the necessary fonts from the *Noto* family are installed in
your system. This setting will produce the following code in the LaTeX
export header:

#+BEGIN_SRC latex :results none :exports code
\babelfont{rm}[Language=Default]{Noto Serif}
\babelfont{sf}[Language=Default]{Noto Sans}
\babelfont{tt}[Language=Default]{Noto Mono}
