...
package, set ~org-latex-fontspec-config~ to ~nil~. This may be the
case when exporting to =beamer=, when you want to use the fonts used
by the template.
...
For example, in the default configuration, the ~math~ font is mapped
to the system font ~TeX Gyre Termes Math~ using the *mandatory*
property ~:font~:
...
with the property ~:features~. Again, taking the example of the
default configuration, the ~mono~ font is defined as:
...
#+BEGIN_SRC latex
\setmonofont{Noto Sans Mono}[Color=blue,Scale=MatchLowercase]
#+END_SRC

The ~:features~ can be a string or a list of strings to improve
readability. An equivalent configuration using a list of strings would
be:
...
character scripts. These are detected by Emacs when exporting to
LuaLaTeX and auxiliary Lua code is generated when a fallback for a
script present in the code is detected. This is not needed when using
XeLaTeX and font specific management packages like ~xeCKJ~ or
~xpinyin~.

The property ~:fallback~ is used for this purpose. It links to an
associative list that maps the script names with their fallback
configuration. Taking the default value of ~org-latex-fontspec-config~
as an example, the =main= font is configured as:
