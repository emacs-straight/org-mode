...
- =LATEX_MULTI_LANG= ::
  ...
  languages. When it is ~nil~, the legacy treatment for multi-language
...
,#+LATEX_MULTI_LANG: polyglossia
  #+END_SRC
  would set the multi-language handling to use polyglossia on a
  document, where the main language (i.e. header, figure, table names)
  and typesetting options use French and indicate that there are
  passages in German and Spanish.
...
#+vindex: org-latex-polyglossia-languages
