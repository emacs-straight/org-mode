...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  (=sans=) and monotype (=mono=) fonts, and can provide fall-backs for
  character sets (for example, ~emoji~, ~han~, ~kana~, etc.) that are
  not provided by the default fonts in your document.  See
...
#+end_example
#+texinfo: @noindent
per language listed in =#+LANGUAGE=. For example:
