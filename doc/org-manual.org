...
and ~FONT~ is either the system font name for that script or a list of
strings. In this case, the first string is the system font name and
the rest are options to apply to this font.
...
(setq org-latex-babel-font-config
      '(("de"
        ...
        ("el-polyton"
         :fonts (("rm" . "Noto Serif")))
        (nil
         :fonts (("tt" . ("DejaVu Sans Mono" "Scale=Matchlocase"))))))
#+END_SRC
...
#+BEGIN_SRC latex :results none :exports code
\babelfont[german]{rm}{CMU Serif}
\babelfont[polytonicgreek]{rm}{Noto Serif}
\babelfont{tt}{DejaVu Sans Mono}[Scale=MatchLowercase]
#+END_SRC

meaning that the text will be set in Serif (~{rm}~) and that text in
German will use the ~CMU Serif~ font and the text in Greek will use
the ~Noto Serif~. The text set in mono-spaced font will use ~DejaVu
Sans Mono~ regardless of the language. The mono-spaced text will be
scaled to match the default font size and produce a better-looking
output.
