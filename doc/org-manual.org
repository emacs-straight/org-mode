...
- =LATEX_MULTI_LANG= ::
  ...
  details about setting up multi-language documents in the [[*Handling
  different languages and character sets in LaTeX exports]]. You can
  set the value globally by customizing ~org-latex-multi-lang~
...
~t~ enables optimised ~fontspec~ configuration[fn:53].
...
~fontspec~ package.  For example, when using XeLaTeX, you can compose
Chinese, Japanese or Korean text using the ~xeCJK~ and ~xpinyin~
packages.  These packages provide the ~\setCKJmainfont{}~,
~\setCJKsansfont{}~ and ~\setCJKmonofnt{}~ commands.  Use ~CJKmain~,
~CJKsans~ and ~CJK mono~ to specify the fonts supporting the Chinese,
Japanese or Korean scripts you need for your documents.  For example:
