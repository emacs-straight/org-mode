...
*Note* that, unless otherwise defined in ~org-latex-classes~, the
theme settings are inserted at the beginning of the preamble, right
after the document class declaration.
...
:end:
#+cindex: @samp{LANGUAGE} keyword, @samp{LATEX_MULTI_LANG} keyword, @samp{SELECT_LANG} keyword
#+vindex: org-latex-multi-lang
#+vindex: org-export-default-language
...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  Additionally, when set the =#+LATEX_MULTI_LANG:= to =babel= or
  =polyglossia= =#+LATEX_MULTI_LANG:= you can use the
  =#+SELECT_LANGUAGE= keyword to switch between languages inside your
  document.
...
,#+SELECT_LANG: <lang>
  ...
  The previous example can be expanded to:
  #+begin_src org
,#+SELECT_LANG: fr
Ce passage est en français.

,#+SELECT_LANG: en-gb
And back to British English.
  #+end_src
...
1. ~:font~ : a string with the system font name. This property is
   *mandatory*.
2. ~:variant~ : a string for the font variant.  The four-letter
   version for the variant corresponds to the family used above for
   ~fontspec~.  It will be translated to
   [[https://en.wikibooks.org/wiki/LaTeX/Fonts][the two-letter codes
   used in LaTeX for the different font families]][fn::Reference
   sources in the Internet always use the two-letter code], according
   to the following table:

   #+CAPTION[Font codes]: Font family codes
   | font family     | 2-letter code | 4-letter code |
   |-----------------+---------------+---------------|
   | Roman (default) | =rm=          | =main=        |
   | Sans serif      | =sf=          | =sans=        |
   | Mono-spaced     | =tt=          | =mono=        |

   When no variant is indicated, the Roman font family (variant
   ~"main"~ or ~"rm"~) is assumed.  It is recommended that you always
   add the variant *explicitly*.
...
#+vindex: org-latex-babel-extra-settings
...
1. ~:variant~ (mandatory) is one of the script codes.  The font family
   translation table in [[#lualatex-polyglossia]] applies and you can
   use 2-letter or 4-letter family codes[fn::Org will translate them
...
Depending on the version of the =babel= package provided by your TeX
distribution and on the specifics of your document, you may need to
add extra parameters when including it. These extra settings are
included through custom variable ~org-latex-babel-extra-settings~.

For example, if your TeX distribution provides an older version of
babel, you may need to add ~import=*~ to the parameters passed to the
=babel= package:

#+begin_src emacs-lisp
(setq org-latex-babel-extra-settings "import=*"
...
#+begin_src latex
\usepackage[bidi=basic,import=*]{babel}
#+end_src

#+texinfo: @noindent
in the LaTeX preamble of your document.

- WARNING :: It is recommended to set this variable in your Emacs
  configuration because it depends on the TeX distribution installed
  in your system. Adding this as a local variable in the document may
  interfere with other peoples' setups.
...
If are writing a multi-lingual document with Latin fonts using
~babel~, for example a Chinese document with passages in American
English, and need to configure the fonts in English passages, use
~org-latex-babel-font-config~ with font definitions for the "en"
language (see [[#babel-fontconfig]]).
