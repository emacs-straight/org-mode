...
- =LATEX_MULTI_LANG= ::
  ...
  configured manually, specifying appropriate LaTeX headers (see
  ~#+LATEX_HEADER~ keyword).  For convenience, if the LaTeX headers
  load =babel= or =polyglossia=, you can use word =AUTO= in the
  package options to automatically specify the document's main
...
#+texinfo: @noindent
  ...
  When it is either ~babel~ or ~polyglossia~, it designates the
  language control package that is imported and the =LANGUAGE= option
  is used to determine the main and secondary languages.
