...
help you, the LaTeX export process prints the list of scripts used in
the document that are outside the ~latin-1~ character set in
the *~*Messages*~* buffer. For example, when you include Cyrillic
text, you will see:
...
At this point, you will need to check, which of the fonts you use does
not include the /offending/ script ad try to substitute it by a
Unicode font that includes it (see above).

When including a font supporting the missing script(s) is not enough,
you can declare the fallback fonts for missing scripts in the
~:fallback~ property based on this list.  It consists of an
property list that maps the script names as detected by Emacs with
...
When you specify ~:features~ for a font family, the fallback
