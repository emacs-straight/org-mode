...
- =LATEX_MULTI_LANG= ::
  ...
  like dates, headings in documents, etc. Possible values are:

  - ~nil~: disable this feature
  - ~"polyglossia"~: Use the Polyglossia package. *Note* that
    ~polyglossia~ will not work with PDFLaTeX. An error will be shown
    before the compilation is attempted.
  - ~"babel"~: Use the Babel package. This package works with all
    LaTeX compilers.

  In order to control locales effectively, according to the LaTeX
  compiler used, the =babel= or =polyglossia= packages must be loaded
  with the appropriate language as argument.  This can be accomplished
  by several means:
  1. The *preferred* way to indicate locales for =lualatex= and
     =xelatex= is to set the package of choice in the
     =#+LATEX_MULTI_LANG= keyword and one or more locales in
     =#+LANGUAGE= using Org language codes. For example:
     #+BEGIN_SRC org
,#+LATEX_COMPILER: lualatex
...
,#+LANGUAGE: en-gb de
     ...
     using the ~babel~ package to manage locales; the document
     includes passages in German, and British English is the main
     locale.
  2. By adding the following code in a ~#+LATEX_HEADER:~ or
     ~#+LATEX_HEADER_EXTRA:~ line:
     #+begin_src org
...
,#+LATEX_HEADER: \usepackage[AUTO]{polyglossia}
     #+end_src
  3. By modifying the ~org-latex-packages-alist~ variable, e.g., with
     either of the following snippets (note that =polyglossia= does
...
(add-to-list 'org-latex-packages-alist
  ...
  The =AUTO= option will be replaced with the list of languages
  declared in the ~#+LANGUAGE:~ keyword for ~babel~ or used to
...
(add-to-list 'org-latex-packages-alist
  ...
  Using ~LATEX_MULTI_LANG~ and a language or language list in
  ~LANGUAGES~ will reflect transparently in the generated LaTeX
  preamble.

  # ~babel~ and ~polyglossia~ also allow you to control fonts on
  # a language basis. Refer to [*** Font control with ~babel~] and
  # [*** Font control with ~polyglossia~] for further information.

  # Note on zh,jp + babel too.
...
# FIXME: change this to fit the LATEX_MULTI_LANG.
#+cindex: @samp{LANGUAGE}, keyword
#+vindex: org-export-default-language
...
including heading, section names and numbering, dates, etc. is using
the =#+LATEX_MULTI_LANG:= [fn:: or the ~org-latex-multi-lang~ custom
variable] and =#+LANGUAGE:= keywords.  For example,
...
would create a document where the main locale is British English. This
document will also have passages with need to be typeset according to
the Spanish locale conventions.

When you use either =babel= or =polyglossia= through the
=#+LATEX_MULTI_LANG:= keyword, you have an additional keyword at your
disposal to switch between languages inside your document.
