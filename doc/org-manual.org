...
- =BEAMER_HEADER= ::
  ...
  - Note :: Do not use locale-handling packages like =babel= or
    =polyglossia= here. Org will warn you and comment them out. Use
    the locale handling features provided by the [[*LaTeX
    Export][LaTeX exporter]] directly.
...
- =LANGUAGE= ::
  ...
  document.  When the =LANGUAGE= keyword is not specified, use the
  value of ~org-export-default-language~ (by default - =en=, American
  English).
...
- =LATEX_MULTI_LANG= ::
  ...
  Define the LaTeX package to use to control locale-dependent features
  like dates, headings in documents, etc.

  Possible values for ~org-latex-multi-lang~ are:

  - ~nil~: Disabled (default value).
  - ~"polyglossia"~: Use the Polyglossia package.
  - ~"babel"~: Use the Babel package.

  Set the package of choice with the =#+LATEX_MULTI_LANG= keyword and
  one or more locales in =#+LANGUAGE= using Org language codes.  For
  example:
  #+BEGIN_SRC org
,#+LATEX_COMPILER: lualatex
...
,#+LANGUAGE: en-gb de
  #+END_SRC
  generates a LaTeX document targeting the ~lualatex~ compiler and
  using the ~babel~ package to manage locales; the document includes
  passages in German, and British English is the main locale.

  *Note* that setting ~org-latex-multi-lang~ or using the
  ~#+LATEX_MULTI_LANG~ keyword will not work with PDFLaTeX.  An error
  will be shown before the compilation is attempted.
...
- =LATEX_FONTSPEC_FEATURES= ::
  ...
  comma-separated list of default settings for ~fontspec~-related
  ...
  For example:
  #+BEGIN_SRC org
,#+LATEX_FONTSPEC_FEATURES: Ligatures=TeX,Scale=MatchLowercase
  #+END_SRC
...
#+vindex: org-latex-compiler
#+cindex: @samp{LATEX_COMPILER}, keyword
You define the LaTeX compiler you want to use with custom variable
~org-latex-compiler~ or with keyword ~#+LATEX_COMPILER~.

The appearance of a compiled LaTeX document controlled by templates
known as LaTeX classes.  By default, the LaTeX export backend uses the
/article/ class.
...
In addition, the LaTeX exporter provides you with different
keywords to control the LaTeX preamble.

#+cindex: @samp{LATEX_DOC_METADATA}, keyword
The LaTeX backend can add metadata to your PDF documents to produce
accessible documents that can be processed with assistive technologies
like screen readers. For example, just to enable tagging, add
#+begin_example
,#+LATEX_DOC_METADATA: tagging=on
#+end_example
to your document.
...
keywords after the document metadata and before the LaTeX class
declaration. Use this option when you want to set the values passed to
packages included in the document class. For example, to use extended
names for the ~xcolor~ package, use
...
document to control locale-specific features line dates, hyphenation,
specific section titles, etc.  See [[*Controlling locales and languages in LaTeX export with =babel= or =polyglossia=.]] for a
detailed description.

You can use custom variable ~org-export-default-language~ to set the
default locale used when exporting to PDF using LaTeX.  Alternatively
you can use keyword ~#+LANGUAGE:~, which accepts a language list
(using Org language codes).  The first language in this list will
define the main locale for the document.

When using LuaLaTeX or XeLaTeX as you LaTeX compiler, you can use the
keyword ~#+LATEX_MULTI_LANG~ to control locale-related configuration
more easily, as described in [[*Controlling locales and languages in
LaTeX export with =babel= or =polyglossia=.]] in more detail.  The
example shown above, using LuaLaTeX as the compiler, with British
English as the locale and using the Babel package to implement it
would look like:
...
*** Controlling locales and languages in LaTeX export with =babel= or =polyglossia=.
    :PROPERTIES:
    :CUSTOM_ID: locale-configuration
    :END:

#+cindex: languages, switching, in @LaTeX{} export
Org will handle locales through well-known LaTeX packages when
exporting to PDF (or LaTeX).
**** Locale handling with ~#+LATEX_MULTI_LANG~

The *preferred* way to control locale-dependent typesetting features,
including heading, section names and numbering, dates, etc. when you
use LuaLaTeX or XeLaTeX is to use the =#+LATEX_MULTI_LANG:= [fn:: or
the ~org-latex-multi-lang~ custom variable] and =#+LANGUAGE:=
...
will create a document where the main locale is British English. This
document will also have passages with need to be typeset according to
the Spanish locale conventions.

**** Legacy locale handling

If you don't use ~#+LATEX_MULTI_LANG~, you have following alternatives
to handle language-related features:

1. You can directly insert the LaTeX code in your LaTeX header
   #+begin_src org
...
2. You can modify the ~org-latex-packages-alist~ variable, e.g., with
   either of the following snippets:
   #+begin_src emacs-lisp
...
(add-to-list 'org-latex-packages-alist
   ...
   #+end_src
   *Note* that =polyglossia= does not work with pdfLaTeX, regardless
   of the configuration method you use. Hence the compiler check.

When enabling The =AUTO= option will be replaced with the list of
languages declared in the ~#+LANGUAGE:~ keyword for ~babel~ or used to
generate the language list for ~polyglossia~.  In either case,
~org-export-default-language~ will be used if you don't use the
~#+LANGUAGE:~ keyword.

If you use ~babel~ and specify more than one language in the
~#+LANGUAGE~ keyword, the list will be inverted before replacing
~AUTO~ to comply with the rule that babel takes the last language in
the list of languages as the main language.

Remember that adding languages to ~AUTO~ as in:
#+BEGIN_SRC  emacs-lisp
(add-to-list 'org-latex-packages-alist
             '("fr,AUTO" "babel"))
  #+END_SRC
is no longer the preferred way of configuring languages and should
be considered *deprecated*.

**** Switching languages

To switch between languages inside your document, you can use the
~#+SELECT_LANG:~ keyword.
...
#+texinfo: @noindent
allows you to switch to language =<lang>= in the document, using *Org*
...
,#+LANGUAGE: en-gb es
,#+macro: today @@latex: \today~ @@

,* Dummy

A dummy test file w/o bibliography. Today is {{{today}}} in Great Britain.

,#+SELECT_LANG: es

,* Un párrafo en castellano

Pues eso, que también lo pruebo. Hoy es {{{today}}} en España.

,#+SELECT_LANG: en-gb

,* Back to =EN_GB=

After our short excursion, we are back to the main language.

#+end_src

You will be warned if the language you pass to ~#+SELECT_LANG:~ is not
included in the languages defined in ~#+LANGUAGES:~.  A language code
not defined in Org will generate an error.

If you need, you may use =#+BEGIN_= - =#+END_= blocks to enclose
passages in a specific language. They will generate a  LaTeX block
enclosed in a ~\begin{<language>}~ / ~\end{<language>}~ pair.  In this
case, however, you will not be able to use Org language names and will
need to know the language name used in the multi-language support
package of your choice.

*** Controlling font setup for LuaLaTeX and XeLaTeX with =fontspec=
:PROPERTIES:
...
*** Controlling font setup for LuaLaTeX and XeLaTeX with =babel=
#+cindex: multiple languages, fonts, babel, in @LaTeX{} export
#+vindex: org-latex-babel-font-config

The =babel= package provides font control features that can be used as
an alternative or in addition to the font control features provided by
the =fontspec= package.

Use the methods described above or the following header in your
document to activate =babel=; for example
...
#+texinfo: @noindent
using the Org language code(s) in the =#+LANGUAGE:= keyword.

If you want to control the fonts used in your document with the
=babel= LaTeX package, use variable ~org-latex-babel-font-config~.
This variable is a list of property lists, which maps Org language
codes to LaTeX font families, system fonts and optional extra
properties.

Each property list has the format:

#+begin_src emacs-lisp
(<lang> :variant <Latex font family> :font <system font name>
        :props <properties to apply to the font>)
#+end_src

The =:variant= can use the /native/ Babel family names (i.e. ~"rm"~,
~"sf"~ and ~"tt"~) or their equivalents for the =fontspec= package
(i.e. ~"main"~, ~"sans"~ and ~"mono"~).

The font mappings for the *main* language need the language code to be
~nil~. The secondary languages use the Org language code.

For example, if you use directory-local variables, you could specify
the font mappings for the main language like this:

#+begin_src emacs-lisp
((org-mode
  . ((org-latex-babel-font-config
      . ((nil :variant "rm" :font "Tex Gyre Pagella")
         (nil :variant "sf" :font "TeX Gyre Heros")
         (nil :variant "tt" :font "TeX Gyre Cursor"))))))
#+end_src

The following Emacs Lisp code in your configuration:

#+begin_src emacs-lisp
(setq org-latex-babel-font-config
   '(("es" :variant "main" :font "DejaVu Serif")
     ("es" :variant "sans" :font "DejaVu Sans")))
#+end_src

#+texinfo: @noindent
will define the fonts for Spanish when it is used as a secondary
language, using the ~fontspec~ variant names.

In addition to the _mandatory_ properties ~:variant~ and ~:font~, you
may add optional properties with ~:props~.  For example:

#+begin_src emacs-lisp
(nil :variant "tt" :font "DejaVu Sans Mono" :props "Scale=MatchLowercase")
#+end_src

- Note :: You may use ~:features~ as an alias for ~:props~.

Consult the [[https://latex3.github.io/babel][=babel= manual]] for further
information.

- Note 2 :: If you define a value for ~org-latex-fontspec-config~, the
  resulting configuration for the ~fontspec~ package will be appended
  to the configuration for the ~babel~ package.

#+vindex: org-latex-babel-provide-extra
The =babel= package uses the features provided by LuaLaTeX as much as
possible. Particularly handy is the possibility to automatically
switch fonts based on the script. For this, you need to add
~"onchar={ids fonts}"~ to the parameters passed to the ~\babelprovide~
command.

Org mode allows you to pass extra parameters to ~\babelprovide~ with
the custom variable ~org-latex-babel-provide-extra~. For example, to
implement script-based font switching, you need to add

#+BEGIN_SRC emacs-lisp
(setq org-latex-babel-provide-extra "onchar={ids fonts}")
#+END_SRC
in your Emacs configuration, or
#+BEGIN_SRC emacs-lisp
(org-latex-babel-provide-extra "onchar={ids fonts}")
#+END_SRC
to your directory-local variables
#+BEGIN_SRC org
# org-latex-babel-provide-extra: "onchar={ids fonts}"
#+END_SRC
to your file-local variables.

*** Controlling font setup for LuaLaTeX and XeLaTeX with =polyglossia=
#+vindex: org-latex-polyglossia-font-config
#+cindex: multiple languages, fonts,polyglossia, in @LaTeX{} export

The LaTeX package =polyglossia= implements two functions. On the one
hand, it helps you control layout and generating dates, document
headers and other elements conforming to typesetting conventions in
different languages and countries. Additionally, it can also help you
define which fonts you want to use depending on the language of a part
of your document. *Note* that =polyglossia= will only work with
=lualatex= or =xelatex=.

To activate =polyglossia=, set the ~LATEX_MULTI_LANG~ option to
=polyglossia= in the document header and specify the language or
languages you are using in the document. Remember that the first
language will be the main language of your document:
...
Fonts will normally be controlled by the =fontspec= package (see
above) and if the fonts you specify ---including fallback fonts---
cover the glyph set you use in your document, you will be done.

However, you can also add fine-grained control to your fonts and
specify specific fonts for specific languages using
~org-latex-polyglossia-font-config~. You don't even need to use
~org-latex-fontspec-config~ if your document doesn't use advanced
features like fallback fonts.

~org-latex-polyglossia-font-config~ is an association
list between Org language names and property lists with the font
specification. You map the Org language name and the font variant to a
system font name with optional features.
...
(setq org-latex-polyglossia-font-config
      '(("es" :font "FreeSerif")
	("es" :variant "sf" :font "FreeSans")
	("es" :variant "tt" :font "DejaVu Sans Mono")
        ("el" :font "DejaVu Serif" :props "Script=Greek")))
#+end_src

#+texinfo: @noindent
will generate the font specification for the three main fonts in
Spanish documents or parts of a document marked as being written in
Spanish and for the Roman font for passages written in Greek.

This will be enough for an Org document with the following header:

#+begin_src org
,#+LANGUAGE: es el
,#+LATEX_COMPILER: lualatex
,#+LATEX_MULTILANG: polyglossia
#+end_src

To read more about =polyglossia=, consult
[[https://texdoc.org/serve/polyglossia/0][the =polyglossia= package
manual]].

- Note 1 :: You may use the long variant names from =fontspec=.  They
  will be automatically translated to variant names understood by
  =polyglossia=.

- Note 2 :: You don't need to specify the =main= or =rm= variant,
  and if you do, it will be silently suppressed in the font definition.

*** Using =babel= for CJK documents with LuaLaTeX or XeLaTeX

You can typeset CJK[fn::Currently Chinese and Japanese documents
are supported.] using ~#+LATEX_MULTI_LANG: babel~ with LuaLaTeX or
XeLaTeX.

**** Creating CJK documents with ~lualatex~
For example, the following header will typeset a document in
Chinese[fn:: Use ~jp~ as the main language for Japanese documents.]:
...
You can add secondary languages, and in this case font mapping must be
handled with ~org-latex-babel-font-config~. For example:
...
needs a font mapping for the text in English; for example, by adding:

#+BEGIN_EXAMPLE
# org-latex-babel-font-config: (
#   ("en" :variant "rm" :font "Free Serif" :props "Scale=MatchLowercase")
#   ("en" :variant "sf" :font "FreeSans"))
#+END_EXAMPLE

to the file-local variables. Depending on the nature of the document,
you may need to redefine the mono-spaced font by adding, for example:

#+BEGIN_SRC org
,#+LATEX_HEADER_EXTRA: \renewfontfamily\ttfamily{DejaVu Sans Mono}%
,#+LATEX_HEADER_EXTRA: [Scale=MatchLowercase]
#+END_SRC
...
Alternatively, you can also create CJK documents with ~xelatex~. In
this case, you will be able to control the fonts you use with
[[*Controlling font setup for LuaLaTeX and XeLaTeX with =fontspec=][a
regular =fontspec= configuration]], adding =CJK...= fonts to the
configuration.  For example, using the following settings in your Org
document header:
...
you should be able to use =xelatex= to generate CJK[fn::Select fonts
that match your language! For example, the fonts in the =Noto= family
for Japanese are the =...CJK JP= specimens.] documents too.
