...
select a Unicode font that covers the scripts you use in your
...
~"AUTO"~ when importing ~babel~ or ~polyglossia~ with a
~#+LATEX_HEADER:~ keyword.
...
When you indicate more than one language in your
document, ~AUTO~ will be replaced with the *first* language.
