...
warn you. When exporting to LaTeX, the export process will always print
a list of scripts in the *~*Messages*~* buffer.

You can declare the fallback fonts for missing scripts based on this
list and using ~:fallback~ property for this purpose.  It links to an
associative list that maps the script names as detected by Emacs with
their fallback configuration.
