...
*** Using LuaLaTeX and =babel= for CJK documents

You can typeset CJK[fn::Currently Chinese and Japanese documents
are supported.] using ~#+LATEX_MULTI_LANG: babel~ with LuaLaTeX. For
example, the following header will typeset a document in Chinese[fn::
use ~jp~ as the main language for Japanese documents.]:
...
#+BEGIN_SRC org
;#+LATEX_HEADER_EXTRA: \renewfontfamily\ttfamily{DejaVu Sans Mono}[Scale=MatchLowercase]
#+END_SRC
