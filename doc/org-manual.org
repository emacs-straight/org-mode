...
- =LATEX_MULTI_LANG= ::
  ...
  The strategy to handle multi-lingual features.  Possible values are
...
~fontspec~ enables optimised ~fontspec~ configuration[fn:53].
...
script for Hebrew script in mono-spaced passages (~:variant "tt"~).
...
When composing Chinese, Japanese or Korean documents, localisation is
...
#+texinfo: @noindent
and the following font configuration:

#+BEGIN_SRC emacs-lisp
((org-mode .
  ((org-latex-fontspec-config .
    (("CJKmain" :font "HaranoAjiMincho")
     ("CJKsans" :font "HaranoAjiGothic")
     ("CJKmono" :font "HaranoAjiGothic"))))))
#+END_SRC

#+texinfo: @noindent
will allow you to typeset a Japanese document. *Note* that you need to
configure the fonts for Japanese documents.
...
#+texinfo: @noindent
*Note:* use the ~Noto ... CJK JP~ fonts for a Japanese document.
