...
#+vindex: org-export-default-language
  ...
  As a general rule, most LaTeX classes include standard font settings
  that work well with languages using the Latin character
  set. Languages using non-Latin character sets will need font
  configuration, which will vary depending on the LaTeX compiler and
  multi-language support package you use.

  The way to handle documents with localisation requirements is set
  using the =LATEX_MULTI_LANG= keyword, or the ~org-latex-multi-lang~
  variable when the keyword is not used.

  =LATEX_MULTI_LANG= is ~nil~ by default. In this case, the LaTeX
  language features need to be configured manually, specifying
  appropriate LaTeX headers (see ~#+LATEX_HEADER~ keyword).  For
  ...
  keyword. Use this setting when you must use =pdflatex= and follow
  the instructions in [[#pdflatex-fonts]] to set up fonts in your
  document.
