...
~fontspec~ enables optimized ~fontspec~ configuration[fn:53].
...
package.  For example, to use roman script instead of italics in
...
document's fonts to the fonts used in the document. To familiarize
...
the ~mono~ font as blue and scaled:
...
For example, assume your ~init.el~ configures the ~main~ font in
...
will include the following fallback definition for the ~main~ font[fn:55]:
...
**** Font configuration when using the ~polyglossia~ package
:PROPERTIES:
...
4. ~:script~: a string the family name, when the information in
   ~org-latex-languages-alist~ is not appropriate.
...
The ~\newfontfamily~ command will define the font family name
based on the ~:script~ property defined in ~org-latex-languages-alist~
font family.  For example, Hindi uses the font family name
~"devanagari"~, derived from the entry for ~"hi~.

Languages that define the ~:script~ that doesn't match the name
expected by ~polyglossia~ need to provide a more accurate description
using the ~:script~ property in
~org-latex-polyglossia-font-config~. For example, if you need to
[[https://tex.stackexchange.com/questions/547807/in-polyglossia-how-can-i-change-the-english-font-in-a-hebrew-document][define
a specific font family for a language using the Latin script]], you
will need to add it manually. The configuration for the linked example
will translate into:
...
(setq org-latex-polyglossia-font-config
      '(("he"
        ...
        ("en" :script "english" :variant "sf"
         :font "Lucida Sans")
        ("en"  :script "english" :variant "tt"
         :font "Lucida Sans Mono")))
#+END_SRC

**** Font configuration when using the ~babel~ package
:PROPERTIES:
...
#+END_SRC
#+texinfo: @noindent
When applied to an Org file with the following settings in the header
...
#+END_SRC
#+texinfo: @noindent
it will generate the following LaTeX preamble code:
...
**** Font configuration for CJK languages with ~babel~ and ~xeCJK~.
:PROPERTIES:
:CUSTOM_ID: babel-xelatex-cjk
:DESCRIPTION: Localisation and font management with xeCJK
:END:

When composing Chinese, Japanese or Korean documents, localization is
provided by ~babel~, while fonts are configured using the ~xeCJK~
package, which provides the ~\setCKJmainfont{}~, ~\setCJKsansfont{}~
and ~\setCJKmonofont{}~ commands.  Use ~org-latex-fontspec-config~ to
define the ~CJKmain~, ~CJKsans~ and ~CJK mono~ fonts supporting the
Chinese, Japanese or Korean scripts you need for your documents. *Note*
that this solution will /only/ work with ~xelatex~.
...
#+texinfo: @noindent
will allow you to typeset a Chinese document. Setting
~org-latex-fontspec-config~ to ~nil~ will produce a working document
with the default fonts provided by the ~xeCJK~ package.

Similarly, you can typeset Japanese documents. The following header
...
#+texinfo: @noindent
will allow you to typeset a Japanese document, using the
=HaranoAjiMincho= and =HaranoAjiGothic= fonts.

You can further adapt the font configuration to your needs. For
example, the following font configuration will produce a Chinese
document typeset with the =Noto= font family:
...
The following font configuration will produce a Japanese document
