...
In Org, entities including emphasized text, links, and super/subscript
can contain hidden components (markers, brackets, link URL, etc.).
When the cursor is adjacent to such hidden text, point can be
ambiguous: at the same apparent cursor position, it can be either
/inside/ or /outside/ the hidden region.  This can make it difficult to
edit precisely.

To alleviate this problem, activate the mode ~org-inside~.  When this
minor mode is active, the appearance is changed when point is "inside"
such an entity, to make it clear where you are.  Appearance changes
can include face (e.g. a subtle background color), cursor type
(e.g. bar instead of box), and unhiding.

Hidden text can be unhidden on demand using {{{kbd(C-c C-c)}}} key
(see [[*The Very Busy {{{kbd(C-c C-c)}}} Key]]) on an entity with
hidden contents, and/or automatically upon entry, with an optional
delay.  ~org-inside~ supports nested entities, applying face appearance
changes to the innermost element.

Note that certain features require Emacs v31 or later.
** Code Evaluation and Security Issues
:PROPERTIES:
