...
When using ~lualatex~ as your =LATEX_COMPILER=, you can control the
fonts used in the PDF and make sure that all characters can be read in
the resulting PDF file.

You can customise ~org-latex-lualatex-font-config~ to map your
...
package manual]] Part II. This variable can also be set in a
~.dir-locals.el~ file placed in the same directory as your ~org~ file.

When you are using =pdflatex= but do not want or need any specific
configuration for the ~fontspec~ package, set
~org-latex-lualatex-font-config~ to ~nil~. This may be the case when
exporting to =beamer=, when you want to use the fonts used by the
template.

~org-latex-lualatex-font-config~ maps the font name used in the
...
script present in the code is detected.

The property ~:fallback~ is used for this purpose. This property links
to an associative list that maps the script names with their fallback
configuration. Taking the default value of
~org-latex-lualatex-font-config~ as an example, the =main= font is
configured as:
...
and use it when defining the main font as:
...
[fn:49] Any font in ~org-latex-lualatex-font-config~ which includes a
