...
be passed to the list after a moderator has approved it[fn:2].  We ask you to read and
respect the
...
~showeverything~, i.e., all file content is visible[fn:6].  This can be configured through the
...
is made visible along with the headline structure above it[fn:7].  Just try it out and you will see
...
-
  ...
  period or a right parenthesis[fn:10], such as =1.=
  or =1)=[fn:11] If you want a list to start with a different
  value---e.g., 20---start the text of the item with =[@20]=[fn:12].
...
- {{{kbd(M-RET)}}} (~org-insert-item~) ::
  ...
  second part becomes the new item[fn:13].  If
...
- {{{kbd(S-UP)}}}, {{{kbd(S-DOWN)}}} ::
  ...
  ~org-support-shift-select~ is off[fn:14].
...
- {{{kbd(M-UP)}}}, {{{kbd(M-DOWN)}}} ::
  ...
  #+kindex: M-UP
  #+kindex: M-DOWN
  Move the item including sub-items up/down[fn:15], i.e., swap
...
=|= is also the column separator[fn:17].
...
you want to be able to assign /names/[fn:21] to fields and columns, you need to reserve the first
...
brackets[fn:22], and activate them as clickable links.
...
the---invisible---bracket at that location[fn:23].  This makes the link
...
section]]= points to a headline with the name =Some section=[fn:24].
...
assigned to the marked object[fn:25].  In the following excerpt from an Org buffer
...
- /Org mode buffers/ ::
  ...
  =ID= property for the link[fn:28].  So using this command in Org buffers potentially creates
  ...
  When ~org-id-link-consider-parent-id~ is ~t~[fn:29], parent =ID= properties
...
- {{{kbd(C-c C-l)}}} (~org-insert-link~) ::
  ...
  - /Completion support/ ::
    ...
    specific completion support for some link types[fn:32].  For example, if you type {{{kbd(f i l
...
Org mode does not maintain TODO lists as separate documents[fn:35].  Instead, TODO items are an
...
adapting the workings of the command {{{kbd(C-c C-t)}}}[fn:37].  When
...
selection character after each keyword, in parentheses[fn:38].  For example:
...
note along with the timestamp, use[fn:41]
...
inserted after the headline as an itemized list, the newest first[fn:42].  When taking a lot
...
=LOGBOOK=[fn:43].  You can also overrule
...
- {{{kbd(S-UP)}}} (~org-priority-up~); {{{kbd(S-DOWN)}}} (~org-priority-down~) ::
  ...
  Increase/decrease the priority of the current headline[fn:45].  Note that
...
below a TODO item, with detailed subtasks on the tree[fn:46].  To keep an overview of the
...
appears[fn:48].  You have to insert the
...
- {{{kbd(C-c C-c)}}} (~org-toggle-checkbox~) ::
  ...
  remove the current one[fn:49].  With a double prefix argument, set it to =[-]=, which is
...
valid tags with corresponding keys[fn:52].
...
| =TIMESTAMP=    | The first active keyword-less timestamp in the entry.[fn:53]   |
...
- {{{var(SUMMARY-TYPE)}}} ::
  ...
  overwrites the property value in parent's property drawer[fn:54].
...
in =org-contrib=[fn:57].  It provides a general API to collect
...
- Diary-style expression entries ::
  ...
  [[info:emacs#Special Diary Entries][Emacs Calendar package]][fn:59].
...
the following configuration in the LaTeX font catalogue:
...
Org mode uses the variable ~org-latex-fontspec-config~ to map the
LaTeX font families in your document to the fonts used in the
generated PDF document.  To familiarize yourself with font families,
fonts and options, read the
[[http://mirrors.ctan.org/macros/unicodetex/latex/fontspec/fontspec.pdf][~fontspec~
package manual]] Part II.

~org-latex-fontspec-config~ can be set in your Emacs
...
For example, consider the following definition for
...
# +texinfo: @noindent
When generating the font prelude for ~lualatex~, the resulting LaTeX commands
will be:
...
#+texinfo: @noindent
and you will use the ~FreeSerif~ font as the default font, the
~FreeSans~ font for sans-serif passages, ~TeX Gyre Cursor~ for
teletype passages (for example, listings) and ~TeX Gyre Termes Math~
for equations or mathematical expressions in general.

If you need extra features for any of these font families, you will
need to add them using the property ~:features~.  For example, if you
want a nicer scaling for the monotype font you could define the ~mono~
font mapping as:
...
The ~:features~ property can be a string or a list of strings to
improve readability.  For example, to make the ~mono~ font family
appear in blue and scaled, you could specify it as:
...
also define the ~mono~ font family as blue and scaled:
...
you can also declare fallback fonts.  ~lualatex~ will then try to use
them for glyphs (characters) that are not included in the fonts you
use in your document.  When this happens, the LaTeX process will
complain and Emacs will warn you.

When exporting to LaTeX, the export process will always print a list
of scripts in the *~*Messages*~* buffer.  You can declare the fallback
fonts for missing glyphs based on this list, using the ~:fallback~
property for this purpose.  This property maps Emacs script names to
font declarations.

The format for the specification is
- ~("<script>" . "<font name>")~ to include a font when a specific
  Emacs script is detected in the document, or
- ~("<script>" . "<font name>:<extra options>")~ to include a font
  with a set of options, when a script is detected.

For example, assume your ~init.el~ file configures the ~main~ font in
...
#+texinfo: @noindent
If Emacs detects characters using what it defines as the ~emoji~ script in the buffer, it
will print a message similar to[fn:61]:

#+BEGIN_EXAMPLE
=> Scripts used in document: (greek symbol emoji)
#+END_EXAMPLE


#+texinfo: @noindent
and using ~org-latex-fontspec-config~ as defined above, it will include
the following fallback definition for the ~main~ font[fn:55]:
...
which will result in the following font specification for =main= in
the LaTeX preamble:
...
Using this fallback definition will enable you to include emojis in
parts of the document that are printed with the main document font.
You may use fallbacks for any font family defined in
~org-latex-fontspec-config~. For example, if your document uses emojis
in the listings only , you need to include the fallback definition for
the =mono= font family, but can omit it for the rest.

Note that:
- you do not need to provide fallback definitions for glyph families
  included in the system fonts you use.
- the script name is a /hint/ provided by Emacs and that it does not
  appear anywhere in the LaTeX preamble.
- when you specify ~:features~ for a font, the fallback definition is
  appended as another feature to the font definition.
...
common features for all fonts, you can define them in a string
...
#+END_SRC
If you need more features, add them to this string to create a
comma-separated list. (Cf.
...
presentation. If you use the same font families for both, you can use
a common font configuration for Org.
...
script for Hebrew script in monospaced passages (~:variant "tt"~).
...
3. ~:props~: a string for extra properties as defined in the
...
1. ~:variant~ (mandatory) is one of the script codes:
   - ~"rm"~ for the roman (serif) font (this is normally the default
...
3. ~:props~ (optional) is a string holding the options to apply to this font.
...
text set in monospaced font will use ~DejaVu Sans Mono~ regardless of
the language. The monospaced text will be scaled to match the default
...
~:provide~ property. This property is a string. It is always ~"import,main"~ for the
main language and defaults for other languages to ~"import"~ if not
present in the configuration; it will generate a line
...
**** Configuration for the ~xeCJK~ package for CJK languages
:PROPERTIES:
...
configured using the ~xeCJK~ package , while localization is provided
by ~babel~ .  *Note* that this solution will /only/ work with
~xelatex~.

Setting ~org-latex-fontspec-config~ to ~nil~ will produce a working
document with the default fonts provided by the ~xeCJK~ package.
...
The ~xeCJK~ package provides the ~\setCKJmainfont{}~,
~\setCJKsansfont{}~ and ~\setCJKmonofont{}~ commands to select specific
fonts.  Use ~org-latex-fontspec-config~ to define the ~CJKmain~,
~CJKsans~ and ~CJKmono~ fonts supporting the Chinese, Japanese or
Korean scripts to match your needs.  For example, the following font
configuration will produce a Chinese document typeset with the =Noto=
font family:
...
a server[fn:62] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:64] with the command
...
to learn to define and use directory local variables. Local variables
supersede your Emacs configuration and are useful to ensure that
everyone is on the same page when collaborating with other people.
...
[fn:61] In this example, the buffer also contained some Greek text and
a couple of equations.

[fn:62] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:63] While creating the agendas, Org mode forces =ID= properties
...
[fn:64] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:65] If the =TBLFM= keyword contains an odd number of dollar
