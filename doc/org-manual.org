...
#+vindex: org-export-default-language
  ...
  multiple-language documents using the LaTeX packages =babel= or
  ...
  When this variable is nil (default), the LaTeX language features
  need to be configured manually, specifying appropriate LaTeX headers
  (see ~#+LATEX_HEADER~ keyword).  For convenience, if the LaTeX
  headers load =babel= or =polyglossia=, you can use word =AUTO= in
  the package options to automatically specify the document's main
  language according to Org's =#+LANGUAGE= keyword:
...
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  ...
  support.  The =LANGUAGE= option will be used to determine the main
  and secondary languages. For example:
...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  Both provide setting up the fonts, including setting them up on a
  per-language basis.
  ...
  # TODO: Consider removing this. This can be treated further down...
...
#+end_example
  ...
  When ~org-latex-multi-lang~ is ~nil~, the first language in the
  =LANGUAGE= keyword (or the value of ~org-export-default-language~ if
  it is not used) will replace appearances of the string ~"AUTO"~ when
  importing ~babel~ or ~polyglossia~ with a ~#+LATEX_HEADER:~ keyword.
...
# Will result in the LaTeX header

# #+BEGIN_SRC latex :results none :exports code
# \usepackage[de,en-gb]{babel}
# #+END_SRC
      ...
      '(("el-polyton"
        ...
        (nil
         :fonts (("rm" :font "CMU Serif")
                 ("tt" :font "DejaVu Sans Mono" :props"Scale=Matchlocase"))))))
#+END_SRC
