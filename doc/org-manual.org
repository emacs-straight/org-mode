...
...
[fn:54] The link is: https://orgmode.org
#+end_example
...
customise LaTeX output.  Setting these keywords works similar to the
...
#+vindex: org-export-default-language
  ...
  in (American) English.  The standard LaTeX document classes assume
  this and provide a default font configuration to deliver a default
  appearance to the output.
  Spelling out dates, section headers for the table of contents,
  etc. as well as captions for figures, tables, listings, etc. are
  defined in this context.  As long as you don't need localisation
  (i.e. adapting or translating these to the document's intended
  linguistic environment), you can use most LaTeX document classes
  as-is with any document that uses the Latin character set.
  # LaTeX
  # document classes include standard font settings that work well in
  # this situation.
  Documents using non-Latin character sets will need
  font configuration[fn:49], which will vary depending on the document
  class, the LaTeX compiler and multi-language support package you
  use.

  To fine-tune the document's appearance provided by the LaTeX
  document class you use, you may need to change the font
  configuration. How this is done depends on the LaTeX compiler you
  use.

  In the case of =pdflatex=, this is achieved by including font
  packages (cf. [[#pdflatex-fonts]]).
  ...
  # When you don't add extra configuration, Org mode will use LaTeX defaults,
  # i.e. American English with Latin fonts.

  # By default, LaTeX only loads the fonts appropriate for Latin
  # characters. Non-Latin characters in the document are often not
  # rendered correctly without loading a custom font. =pdflatex= relies
  # on LaTeX packages you need to include using the =#+LATEX_HEADER:=
  # keyword.  =lualatex= and =xelatex= handle custom fonts using either
  # =fontspec=, =polyglossia= or =babel= as described below. We
  # recommend selecting Unicode fonts that cover the character sets of
  # in the languages used in your document.

  The languages used in a document are specified using =LANGUAGE=
  keyword. Use the =LATEX_MULTI_LANG= keyword to set the way
  to handle localisation and font configuration.

  =LATEX_MULTI_LANG= is ~nil~ by default to handle localisation
  manually.  In this case you need to configure LaTeX language
  features manually in LaTeX headers (see ~#+LATEX_HEADER~ keyword).
  ...
  main language. Org's will translate it according to the value of the
  =#+LANGUAGE= keyword. Use this setting when you must use =pdflatex=
  and follow the instructions in [[#pdflatex-fonts]] to set up fonts
  in your document.
...
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  ...
  with Spanish headers. The =LATEX_HEADER= would be translated to
  #+BEGIN_EXAMPLE
...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  Note that ~polyglossia~ does not work with =pdflatex= compiler.  We
...
[[https://tug.org/FontCatalogue/erewhon/][Erewhon]] font, you will see
the following configuration in the linked page:
...
include that.  Other commands need to be included manually:
...
*** Controlling the font configuration for LuaLaTeX and XeLaTeX
:PROPERTIES:
...
control the fonts used in the PDF using the =fontspec=. Remember that
=polyglossia= will load =fontspec= automatically. Setting variable
~org-latex-multi-lang~ to ~t~ enables optimised =fontspec=
configuration.

Org mode will load =fontspec= for =babel=. However, font configuration
will use a different mechanism described [[#babel-fontconfig][below]].

**** Controlling the configuration of the =fontspec= package
:PROPERTIES:
...
document's fonts to the fonts used in the document. To familiarise
yourself with fonts and options, read the
[[http://mirrors.ctan.org/macros/unicodetex/latex/fontspec/fontspec.pdf][~fontspec~
package manual]] Part II.  This variable should be set in your Emacs
initialisation code, in the file local variables or in the directory
local variable file ~.dir-locals.el~[fn:50] .
...
~org-latex-fontspec-config~ maps the font name used in the
=\set...font{}= commands defined by the =fontspec= package with the
font name and optional font features.

For example, if you want to map the ~math~ font to the system font
~TeX Gyre Termes Math~, you should use the *mandatory* property
~:font~. Consider the following definition for
...
When generating the font prelude for ~lualatex~, the resulting commands
will be:
...
If you need extra features for your font, you will need to add them
with the property ~:features~.  For example, if you want a nicer
scaling for the monotype font you could define the ~mono~ font as:
...
The ~:features~ can be a string or a list of strings to improve
readability.  For example, to make the ~mono~ font appear in blue and
scaled, you could specify it as:
...
as a comma-separated list. The following code also would also define
...
For example, assume you configure the =main= font in
~org-latex-fontspec-config~ as:
...
If Emacs detects characters belonging to the ~emoji~ character set, it
will include the following fallback definition for the =main= font[fn:51]:
...
and, then, generate the following font specification for =main=.

#+BEGIN_SRC latex
\setmainfont{TeX Gyre Termes}[RawFeature={fallback=fallback_main}]
\setsansfont{FreeSans}
\setmonofont{TeX Gyre Cursor}[Scale=MatchLowercase]
\setmathfont{TeX Gyre Termes Math}
#+END_SRC
...
are printed with the main document font.

Note that when you specify ~:features~ for a font, the fallback
definition is appended as another feature to the font definition.
The format for the specification is
1. ~("<script>" . "<font name>")~ to activate a font for a specific
   script, or
2. ~("<script>" . "<font name>:<extra options>")~ to activate a font
   with a set of options for a specific script
...
#+vindex: org-latex-fontspec-default-features
Additionally, you can define common features for all fonts using the
~org-latex-fontspec-default-features~ variable. Its default value is
`nil' meaning that no defaults are defined. If you need to default
common features for all fonts, you can define them as an associative
list of strings (FEATURE . VALUE) in this variable. For example:

#+BEGIN_SRC elisp :results none :exports code :eval never
(setq org-latex-fontspec-default-features
      '(("Numbers". "OldStyle")))
#+END_SRC

or its equivalents as a file or directory local variable will generate
the following LaTeX code in the file header:
...
#+END_SRC
If you need more features, add them to this list.

**** Controlling font configuration when using the =polyglossia= package
:PROPERTIES:
...
If you use the =polyglossia= package in multi-lingual documents, it
will load for you the =fontspec= package. Any general settings that
...
basis. For example, the document will use using the =Miriam Mono CLM=
script for Hebrew script in mono-spaced passages (=:variant
"tt"=). You will see the following in the exported LaTeX:
...
="he"= for Hebrew, ="en"= for English, etc.[fn:52]) to a list
of properties.
...
2. =:variant=: a string for the font variant (e.g. "=sf=" for Sans
   ...
   codes used in LaTeX for the different font families]].
...
when exporting to babel.  Each entry maps a language name to a
~:fonts~ property.  Use ~nil~ as the language name for the fonts
used by the default language:

#+begin_example
(ORG-LANG :fonts FONTS)
#+end_example

The ~:fonts~ property list has the form of an associative list, where
...
- ~"rm"~ for the roman (serif) font (this is normally the default font
  for text passages),
- ~"sf"~ for the sans serif font, and
- ~"tt"~ for the teletype (monospaced) font.

and ~FONT-PLIST~ is a property list, with a mandatory property ~:font~
for the system font name for that script and an optional property ~:props~.
The property ~:props~ is either a string or a list of strings that represent
options to apply to this font.

For example, for a document with text in German and Greek,
where the main language is German, this variable could be
...
implying that text set in Serif (~{rm}~) will use the ~CMU Serif~ font
unless it is in Greek, where the ~Noto Serif~ font will be used. The
text set in mono-spaced font will use ~DejaVu Sans Mono~ regardless of
the language. The mono-spaced text will be scaled to match the default
font size and produce a better-looking output.

You can add an optional ~:provide~ property to any language mapping
except the default mapping. The ~:provide~ property is a string and
will default to ~"import"~ if not present. It will generate a

#+begin_example
\babelprovide[provide]{lang}
#+end_example

line with additional configuration when needed. The main language will
always use ~"import,main"~. For example:

#+BEGIN_SRC emacs-lisp :results none :exports code
(setq org-latex-babel-font-config
      '((nil
         :fonts (("rm" :font "FreeSerif")))
        ("pt"
         :fonts (("rm" :font "DejaVu Sans"))
         :provide "onchar=ids fonts")))
#+END_SRC

applied to an org file with
#+BEGIN_SRC org :results none :exports code
...
to set a document in Greek and Portuguese using the ~DejaVu Sans~ font
for the Portuguese text and ~FreeSerif~ for Greek.
...
By calling a named code block[fn:53] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:53].  Functional mode.  Org
...
Emacs[fn:56].  With Org's support, you have to indent all lines to
...
a server[fn:57] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:59] with the command
...
[fn:50] Consult [[info:emacs#Directory Variables][Directory Variables]]
to learn to define and use directory local variables.

[fn:51] Any font in ~org-latex-fontspec-config~ which includes a
fallback for the =emoji= character set will produce equivalent code
inside the =\directlua{}= code block.

[fn:52] The language name can be the language as written out in the
font declaration or the language identifier used in
~org-latex-languages-alist~.

[fn:53] Actually, the constructs =call_<name>()= and =src_<lang>{}=
are not evaluated when they appear in a keyword (see [[*Summary of
In-Buffer Settings]]).

[fn:54] In the following example, attempting to evaluate
...
[fn:55] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:56] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:57] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:58] While creating the agendas, Org mode forces =ID= properties
...
[fn:59] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:60] If the =TBLFM= keyword contains an odd number of dollar
