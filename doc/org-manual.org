...
- =LATEX_MULTI_LANG= ::
  ...
  are ~"polyglossia"~, ~"babel"~, ~t~ or ~nil~.

  By default, it is ~nil~ and multi-language documents are controlled
  by importing the LaTeX package to control multi-language features
  with the ~#+LATEX_HEADER~ keyword. Use =AUTO= in the package options
  to include language information, for example:
  #+BEGIN_EXAMPLE
#+LANGUAGE: es
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  ...
  When ~org-latex-multi-lang~ is ~nil~, all options and
  features are controlled by the user in ~#+LATEX_HEADER:~ options.

  When it is ~t~, only font control features implemented in the
  =fontspec= package are activated, when the LaTeX compiler is
  =lualatex= or =xelatex=.
