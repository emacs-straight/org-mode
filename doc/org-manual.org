...
with the property ~:props~ [fn:: You may also use ~:features~ as an
alias for ~:props~. (cf. ~org-latex-fontspec-default-features~
...
would also define the ~mono~ font as blue and scaled:
...
you. To help you fix the font setup, Org will include a list of
/problematic/ fonts with the Emacs scripts they don't support. For
example:
...
Most LaTeX document classes use the ~lm...~ fonts by default. These
fonts are installed with your LaTeX distribution. Most of the times,
replacing these fonts by system fonts supporting the missing scripts
will be enough (the Cyrillic script in the example above)[fn::Replace
Cyrillic with your specific needs. You just need to find a Unicode
font supporting the scripts you use in the document.].
...
fonts, you need a substitute font for the family that needs them.

Turning back to the example above. The ~Free...~ fonts provide
reasonable substitutes for the default ~lm...~ fonts. With this in
mind you could define ~org-latex-fontspec-config~ as follows:
