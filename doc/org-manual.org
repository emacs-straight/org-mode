...
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  ...
  configure the font defaults using LaTeX's ~fontspec~ package.  Note
  that ~fontspec~ can configure the fonts either globally (for all the
  languages used in the document), or for different character sets
  (for example, ~emoji~, ~han~, ~kana~).  See [[#lualatex-fontspec]]
  ...
  multi-language support.  The =LANGUAGE= option will be used to determine
