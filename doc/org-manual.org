...
...
[fn:56] The link is: https://orgmode.org
#+end_example
...
- =LATEX_MULTI_LANG= ::
  ...
  [[*Handling different languages and character sets in LaTeX
  exports]] section. Using the =LATEX_MULTI_LANG= option implies
  setting the ~#+LATEX_MULTI_LANG:~ keyword or the ~org-multi-lang~
  variable. Setting the keyword supersedes the variable value.
...
#+vindex: org-export-default-language
  ...
  need localisation (i.e. adapting or translating these to the
  ...
  compiler and multi-language support package you use[fn:49]. Consult
  the
  [[https://en.wikibooks.org/wiki/LaTeX/Internationalization][internationalisation
  section]] of the
  ...
  =babel= and =polyglossia= to localise document features like text
  ...
  handle localisation manually, configuring LaTeX language features in
...
#+LATEX_HEADER: \usepackage[AUTO]{babel}
  ...
  Spanish headers. The =LATEX_HEADER= would be translated [fn:50] to
  #+BEGIN_EXAMPLE
...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  Consult the package manuals to get the language name[fn:51] for the
  LaTeX markup.
  ...
  not work with =pdflatex= compiler. In this case you can control
...
variable ~org-latex-multi-lang~ or the =LATEX_MULTI_LANG= option to
~t~ enables optimised ~fontspec~ configuration.

~polyglossia~ and ~babel~ load ~fontspec~ internally. When using
~polyglossia~, you can combine ~fontspec~ and ~polyglossia~-specific
configuration.  Font configuration will use a different mechanism
[[#babel-fontconfig][when using ~babel~]].
...
local variable file ~.dir-locals.el~[fn:52] .
...
to ~nil~ (this is the default value).  This is, for example, the case
when you export to ~beamer~ and want to rely on the fonts defined in
...
will include the following fallback definition for the =main= font[fn:53]:
...
="he"= for Hebrew, ="en"= for English, etc.[fn:54]) to a list
of properties.
...
except the default mapping. The ~:provide~ property is a string. It is
always ~"import,main"~ for the main language and defaults for other
languages to ~"import"~ if not present in the configuration; it will
generate a line
...
#+end_example
#+texinfo: @noindent
per language listed in =#+LANGUAGE=. For example, if you can detect
the language a passage is written by the characters, you can use the
babel option =onchar=ids fonts":
...
#+END_SRC
#+texinfo: @noindent
applied to an Org file with the following settings in the header
...
#+END_SRC
#+texinfo: @noindent
to typeset a document in Greek and Portuguese using the ~DejaVu Sans~ font
...
By calling a named code block[fn:55] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:55].  Functional mode.  Org
...
Emacs[fn:58].  With Org's support, you have to indent all lines to
...
a server[fn:59] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:61] with the command
...
[fn:49] For example, in a document is that is intended for an American
...
[fn:50] Org will translate the language codes to the language
...
[fn:57] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:58] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:59] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:60] While creating the agendas, Org mode forces =ID= properties
...
[fn:61] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:62] If the =TBLFM= keyword contains an odd number of dollar
