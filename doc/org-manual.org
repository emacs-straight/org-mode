...
=polyglossia= or =babel= load =fontspec=.
...
If you use the =polyglossia= package in multi-lingual documents, font
usage will be controlled by ~org-latex-polyglossia-font-config~. Its
default value is ~nil~, meaning that no extra font configurations are
generated. =polyglossia= controls fonts on a per-language, per-script
basis. For example, using the =FreeMono= script for Hebrew script in
mono-spaced passages, is achieved with the following definition in
LaTeX:
...
The variable ~org-latex-polyglossia-font-config~ contains a list of
definitions for the different fonts you need in your PDF output.
Each definition is a property list (~plist~). To generate the
definition for the monospaced Hebrew font above, you will need to add

#+BEGIN_SRC emacs-lisp :results none :exports code
  ("hebrew"  :font "FreeMono" :variant "tt" :extras "[Script=Hebrew]")
#+END_SRC

to ~org-latex-polyglossia-font-config~. The properties you can define
are:
