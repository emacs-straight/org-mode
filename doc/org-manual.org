...
with the property ~:props~ [fn:: You may also use ~:features~ for
~:props~. (cf. ~org-latex-fontspec-default-features~ below)].  For
example, if you want a nicer scaling for the monotype font you could
define the ~mono~ font as:
...
The ~:props~ property can be a string or a list of strings to improve
...
When using =lualatex=, the LaTeX export process prints the list of
scripts used in the document that are outside the ~latin-1~ character
set in the *~*Messages*~* buffer. For example, when you include
Cyrillic text, you will see:

#+begin_example
=> Scripts used in document: (cyrillic)
#+end_example

If the system fonts you use to produce the PDF output don't support a
specific script, the LaTeX compiler will fail and Emacs will warn
you. To help you, Org will include a list of /problematic/ fonts with
the Emacs scripts they don't support. For example:

#+begin_example
Scripts missing:
For font lmmono10-regular: cyrillic
For font lmroman10-regular: cyrillic
For font lmsans10-regular: emoji
#+end_example

Most LaTeX document classes include the ~lm...~ fonts by
default. These fonts are installed with your LaTeX distribution. Most
of the times, replacing these fonts by system fonts supporting the
missing scripts will be enough (the Cyrillic script in the example
above)[fn::Replace Cyrillic with your specific needs. You just need to
find a Unicode font supporting your needs.].

When one or more scripts are not supported by your default choice (for
example, the ~emoji~ script in the example above), you can declare
fallback fonts with the ~:fallback~ property for the specific font
family based on this list.

The ~:fallback~ property is list that maps Emacs script names to their
fallback system font and optional extra configuration.

For the example above, you would need to find Unicode font substitutes
for the ~lmroman...~ and ~lmmono...~ fonts, because then lack the
Cyrillic script. In the case of the ~lmsans...~ font, you need to add
a fallback font for the ~emoji~ script. In order to allow fallback
fonts, that you need a substitute font too.

The ~Free...~ fonts provide reasonable substitutes for the default
~lm...~ fonts. With this in mind you could define
~org-latex-fontspec-config~ as follows:
...
it will include the following fallback definition for the ~sans~
font[fn:55]:
...
the following font specification for =sans=.

#+BEGIN_SRC latex
\setmainfont{FreeSerif}
\setsansfont{FreeSans}[RawFeature={fallback=fallback_sans}]
