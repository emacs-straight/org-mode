...
4. ~:script~: a string the family name, when the ~:polyglossia~
   property in ~org-latex-languages-alist~ is not appropriate.
...
based on the ~:polyglossia~ property defined in ~org-latex-languages-alist~
font family.  For example, Spanish uses the font family name
...
~org-latex-polyglossia-font-config~. For example,
...
[[https://rishi.app/blog/writing-hindi-using-devanagari-package-in-overleaf/][this is not always needed]].
