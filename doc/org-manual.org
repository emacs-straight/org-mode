...
local variable file ~.dir-locals.el~[fn:51] .
...
will include the following fallback definition for the =main= font[fn:52]:
...
="he"= for Hebrew, ="en"= for English, etc.[fn:53]) to a list
of properties.
...
By calling a named code block[fn:54] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:54].  Functional mode.  Org
...
Emacs[fn:57].  With Org's support, you have to indent all lines to
...
a server[fn:58] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:60] with the command
...
[fn:56] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:57] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:58] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:59] While creating the agendas, Org mode forces =ID= properties
...
[fn:60] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:61] If the =TBLFM= keyword contains an odd number of dollar
