...
- =LATEX_POLYGLOSSIA_LANGS= ::

  #+cindex: @samp{LATEX_POLYGLOSSIA_LANGS}, keyword
  #+vindex: org-latex-polyglossia-languages
  The languages present in a document. When using the =xelatex= or
  =lualatex= compilers to produce the PDF, the backend will load the
  =polyglossia= package when this variable is not ~nil~ and set the
  main language with the =LANGUAGE= option and the secondary languages
  indicated by this option.

  For example:
  #+BEGIN_SRC org :results none :exports code
,#+LANGUAGE: fr
,#+LATEX_POLYGLOSSIA_LANGS: de,es,AUTO
  #+END_SRC
  would set the main language of the document (i.e. header, figure,
  table names) and typesetting options to French and indicate that
  there are passages in German and Spanish.
...
=Polyglossia= is controlled by the ~org-latex-polyglossia-languages~
variable. It uses the same =babel= classic syntax (but note that /this
is not/ the actual polyglossia syntax), with =AUTO= denoting the main
language.  For example, suppose a document declares Polytonic Greek as
the primary language, and French as the secondary language.
#+vindex: org-latex-polyglossia-languages

#+begin_example
,#+LANGUAGE: el-polyton
,#+LATEX_POLYGLOSSIA_LANGS: french,AUTO
#+end_example
