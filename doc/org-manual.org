...
...
[fn:52] The link is: https://orgmode.org
#+end_example
...
- =LANGUAGE= ::
  ...
  with the appropriate language as argument.
...
- =LATEX_MULTI_LANG= ::
  ...
  ~"babel"~ or ~"polyglossia"~, the package is imported and the
  =LANGUAGE= option is used to determine the main and secondary
  languages. When it is ~nil~, multi-language documents are controlled
  by importing the package with the ~#+LATEX_HEADER~ keyword.
...
,#+LATEX_MULTI_LANG: polyglossia
  ...
  sets multi-language handling to use ~polyglossia~ on a document, where
  the main language (i.e. header, figure, table names) and typesetting
  options use French: it also indicates that there are passages in German
  and Spanish.
...
When generating the font prelude for ~lualatex~, the resulting command
will be:
...
with the property ~:features~.  For example, you could define the
~mono~ font as:
...
You can also add fallback fonts for specific font names for specific
character scripts.  These are detected by Emacs during the export
process.
...
#+BEGIN_SRC emacs-lisp :results none :exports code
  ("hebrew"  :font "FreeMono" :variant "tt" :props "Script=Hebrew")
#+END_SRC
...
By calling a named code block[fn:51] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:51].  Functional mode.  Org
...
Emacs[fn:54].  With Org's support, you have to indent all lines to
...
a server[fn:55] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:57] with the command
...
[fn:51] Actually, the constructs =call_<name>()= and =src_<lang>{}=
are not evaluated when they appear in a keyword (see [[*Summary of
In-Buffer Settings]]).

[fn:52] In the following example, attempting to evaluate
...
[fn:53] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:54] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:55] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:56] While creating the agendas, Org mode forces =ID= properties
...
[fn:57] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:58] If the =TBLFM= keyword contains an odd number of dollar
